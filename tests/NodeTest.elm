...
suite =
    describe "Node"
        [ describe "addAfter"
            [ test "addAfter beginning of an empty list" <|
                \_ ->
                    addAfter 0 ( 1, 'a' ) root
                        |> expectSuccessWithChildren [ init 'a' [ 1 ] ]
            , test "addAfter timestamp appends node" <|
                \_ ->
                    addAfter 0 ( 1, 'a' ) root
                        |> Result.andThen (addAfter 1 ( 2, 'b' ))
                        |> expectSuccessWithChildren
                            [ init 'b' [ 2 ], init 'a' [ 1 ] ]
            , test "addAfter timestamp appends consecutive nodes" <|
                \_ ->
                    addAfter 0 ( 1, 'a' ) root
                        |> Result.andThen (addAfter 1 ( 2, 'b' ))
            ...
            , test "addAfter non existant node fails" <|
                \_ ->
                    Expect.equal
                        (addAfter 10 ( 1, 'a' ) root)
                        (Err NotFound)
            , test "addAfter negative timestamp fails" <|
                \_ ->
                    Expect.equal
                        (addAfter -10 ( 1, 'a' ) root)
                        (Err BadTimestamp)
            , test "addAfter twice fails" <|
                \_ ->
                    addAfter 0 ( 1, 'a' ) root
        ...
        , describe "delete"
            [ test "delete marks node as tombstone" <|
                \_ ->
                    addAfter 0 ( 1, 'a' ) root
                        |> Result.andThen (delete 1)
                        |> expectSuccessWithChildren [ tombstone [ 1 ] ]
            , test "delete non existing fails" <|
                \_ ->
                    addAfter 0 ( 1, 'a' ) root
                        |> Result.andThen (delete 10)
                        |> Expect.equal (Err NotFound)
            , test "delete with negative timestamp fails" <|
                \_ ->
                    addAfter 0 ( 1, 'a' ) root
                        |> Result.andThen (delete -10)
                        |> Expect.equal (Err BadTimestamp)
            , test "delete with timestamp 0 fails" <|
                \_ ->
                    addAfter 0 ( 1, 'a' ) root
